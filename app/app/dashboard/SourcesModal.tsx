...
type ClusteringProps = ClusteringState & { start: () => void; reset: () => void };
...
export function SourcesModal({ connections, onConnectionChange, onClose, clustering }: SourcesModalProps) {
    ...
    >
      <div
        ...
        style={{
          ...
          borderRadius: "12px",
          boxShadow: "0 24px 80px rgba(0,0,0,0.8), 0 0 0 1px rgba(232,165,71,0.06)",
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            width: "300px",
            height: "200px",
            background: "radial-gradient(ellipse, rgba(232,165,71,0.06) 0%, transparent 70%)",
          }}
        />
        ...
        >
          <div>
            <h2 style={{ fontFamily: "Syne, sans-serif", fontSize: "16px", fontWeight: 700, letterSpacing: "-0.02em", color: "#E8E9ED" }}>
              Knowledge Sources
            </h2>
        ...
        <div className="flex-1 overflow-y-auto px-7 py-5" style={{ scrollbarWidth: "none" } as React.CSSProperties}>
          ...
          {hasConnections && (
            <section className="mb-7">
              <div className="mb-4" style={{ borderTop: "1px solid rgba(58,61,71,0.4)" }} />
        ...
        {/* Footer */}
        <div className="px-7 py-4 flex items-center justify-between" style={{ borderTop: "1px solid rgba(58,61,71,0.4)" }}>
          <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "9px", letterSpacing: "0.18em", color: "#3A3D47", textTransform: "uppercase" }}>
            Sources sync every 15 min
          </span>
          <button
            onClick={onClose}
            style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "10px", letterSpacing: "0.15em", color: "#9CA0AC", textTransform: "uppercase" }}
...
function DriveCard({ connection, onConnectionChange }: { connection: DriveConnection; onConnectionChange: () => void }) {
  ...
  return (
    ...
    >
      {/* Drive icon */}
      <div className="w-8 h-8 rounded flex items-center justify-center flex-shrink-0" style={{ background: "rgba(107, 176, 122, 0.12)" }}>
        <DriveIcon />
      </div>

      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span style={{ fontFamily: "DM Sans, sans-serif", fontSize: "13px", fontWeight: 400, color: "#E8E9ED" }} className="truncate">
            {connection.google_account_email}
          </span>
          <div className="flex items-center gap-1 flex-shrink-0">
            <div className="w-1.5 h-1.5 rounded-full bg-[#6BB07A]" style={{ boxShadow: "0 0 5px rgba(107,176,122,0.7)" }} />
            <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "9px", letterSpacing: "0.15em", color: "#6BB07A", textTransform: "uppercase" }}>
              Active
            </span>
          </div>
        </div>
        <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "9px", color: "#9CA0AC", letterSpacing: "0.1em", marginTop: "2px" }}>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <button
          ...
          style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "9px", letterSpacing: "0.15em", color: "#E8A547", textTransform: "uppercase", border: "1px solid rgba(232,165,71,0.25)" }}
        >
          {resyncing ? "…" : "Resync"}
        </button>
        <button
          ...
          style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "9px", letterSpacing: "0.15em", color: "#9CA0AC", textTransform: "uppercase", border: "1px solid rgba(58,61,71,0.4)" }}
        >
          {removing ? "…" : "Remove"}
        </button>
      </div>
...
function ConnectGoogleDriveButton({ secondary = false }: { secondary?: boolean }) {
  ...
  return (
    ...
    >
      <div className="w-7 h-7 rounded flex items-center justify-center flex-shrink-0" style={{ background: secondary ? "rgba(58,61,71,0.4)" : "rgba(232,165,71,0.1)" }}>
        <DriveIcon size={14} />
      </div>
      <div className="flex-1 text-left">
        <div style={{ fontFamily: "DM Sans, sans-serif", fontSize: "13px", fontWeight: secondary ? 300 : 500, color: secondary ? "#9CA0AC" : "#E8E9ED" }}>
...
function ClusteringSection({ clustering }: { clustering: ClusteringProps }) {
  if (clustering.status === "done") {
    return (
      <div className="px-4 py-3 rounded flex items-center justify-between" style={{ background: "rgba(107,176,122,0.06)", border: "1px solid rgba(107,176,122,0.2)" }}>
        <div className="flex items-center gap-2">
          <div className="w-1.5 h-1.5 rounded-full bg-[#6BB07A]" style={{ boxShadow: "0 0 5px rgba(107,176,122,0.7)" }} />
          <span style={{ fontFamily: "DM Sans, sans-serif", fontSize: "13px", fontWeight: 400, color: "#E8E9ED" }}>Map built</span>
          {clustering.totalTopics && (
            <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "9px", color: "#9CA0AC", letterSpacing: "0.1em" }}>
              · {clustering.totalTopics} topics
            </span>
          )}
        </div>
        <Link href="/query" style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "9px", letterSpacing: "0.15em", color: "#E8A547", textTransform: "uppercase", textDecoration: "none" }} className="hover:text-[#F5B25F] transition-colors">
          View in Query →
        </Link>
      </div>
    );
  ...
  if (clustering.status === "running") {
    return (
      <div className="px-4 py-3 rounded flex items-center gap-3" style={{ background: "rgba(232,165,71,0.04)", border: "1px solid rgba(232,165,71,0.15)" }}>
        <SpinnerIcon />
        <div className="flex-1">
          <span style={{ fontFamily: "DM Sans, sans-serif", fontSize: "13px", fontWeight: 300, color: "#E8E9ED" }}>Building knowledge map</span>
          {(clustering.totalTopics ?? 0) > 0 && (
            <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "9px", color: "#9CA0AC", letterSpacing: "0.1em", marginLeft: "8px" }}>
  ...
  if (clustering.status === "failed") {
    return (
      <div className="space-y-2">
        <div className="px-4 py-2.5 rounded flex items-center gap-2" style={{ background: "rgba(201,80,75,0.06)", border: "1px solid rgba(201,80,75,0.2)" }}>
          <span style={{ color: "#C9534B", fontSize: "12px" }}>✕</span>
          <span style={{ fontFamily: "DM Sans, sans-serif", fontSize: "12px", fontWeight: 300, color: "#C9534B", flex: 1 }}>
            {clustering.error ?? "Clustering failed"}
          </span>
          <button onClick={clustering.reset} style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "9px", letterSpacing: "0.15em", color: "#9CA0AC", textTransform: "uppercase" }} className="hover:text-[#E8E9ED] transition-colors">
            Dismiss
          </button>
        </div>
  ...
  return (
    ...
    >
      <div className="text-left">
        <div style={{ fontFamily: "DM Sans, sans-serif", fontSize: "13px", fontWeight: 500, color: "#E8E9ED" }}>Build Knowledge Map</div>
        <div style={{ fontFamily: "DM Sans, sans-serif", fontSize: "11px", fontWeight: 300, color: "#9CA0AC", marginTop: "1px" }}>Cluster your docs into topics for AI querying</div>
      </div>
      <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="#E8A547" strokeWidth="1.4" className="flex-shrink-0 ml-3">
        <line x1="2" y1="7" x2="12" y2="7" /><polyline points="8,3 12,7 8,11" />
      </svg>
    </button>
  );
...
function SectionLabel({ text }: { text: string }) {
  return (
    <div className="mb-3" style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "9px", letterSpacing: "0.18em", color: "#9CA0AC", textTransform: "uppercase" }}>
      {text}
    </div>
  );
}
...
function SpinnerIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="#E8A547" strokeWidth="1.5" className="flex-shrink-0" style={{ animation: "spin 1s linear infinite" }}>
...
function SoonPill() {
  return (
    <div className="px-1.5 py-0.5 rounded" style={{ background: "rgba(58,61,71,0.5)", fontFamily: "JetBrains Mono, monospace", fontSize: "8px", letterSpacing: "0.15em", color: "#3A3D47", textTransform: "uppercase", flexShrink: 0 }}>
      Soon
    </div>
  );
}
